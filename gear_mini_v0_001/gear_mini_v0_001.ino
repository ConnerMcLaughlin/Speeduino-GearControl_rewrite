...
#if !(defined(__GNUC__) || defined(__CLANG__)) /*If there are more compilers that use the ellipsis operator, add 'em here*/
  #error Either find an alternative to ellipsis in "switch case" or use a gcc/clang compiler
#endif 
       ...
       if ((mainLoopCount & 31) == 1) //Every 32 loops read analog and digital inputs
            {
              for (byte Achan = 1; Achan <17 ; Achan++)
                    if (pinAin[Achan] < 255) 
                      readAnalog(Achan);        // read the analog inputs

              if(!BIT_CHECK(currentStatus.testIO_hardware, 1))
                  for (byte Dinchan = 1; Dinchan <17 ; Dinchan++)
                        if (pinIn[Dinchan] < 255) 
                           readDigitalIn(Dinchan);        // if pin is not set to 0 then is in use so read the digital inputs          
              driveOutputs();
              selectorInput(); 
              gearStatus();
              gearOutput();
